...
<title>@yield('title') Comisionados</title>
            ...
            <div class="col-lg-12">
                <div class="card mb-4">
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                        ...
                        <a href="{{ url('comisionado/pdf') }}" class="btn btn-sm btn-danger" target="_blank" id="pdfButton">
                        {{ ('PDF') }}
                        </a>

                        <h2 class="font-weight-bold text-primary" style="margin-left: 6%;">Gestión de Comisionados</h2>
                            @can('crear-comisionado')
                                <form action="{{ route('comisionado.create') }}" method="get" style="display:inline;">
                        ...
                        <table class="table align-items-center table-flush" id="dataTable">
                            ...
                            <tbody>
                                @foreach ($comisionados as $comisionado)
                                    <tr>
                                        <!-- <td class="font-weight-bold text-Secondary">{{ $comisionado->id}}</td> -->
                                        <td class="font-weight-bold text-Secondary">{{ $comisionado->cedula }}</td>
                                        <td class="font-weight-bold text-Secondary">{{ $comisionado->nombres }}</td>
                                        ...
                                        <td>
                                            ...
                                            <div style="display: flex; justify-content: center;">
                                                @can('editar-comisionado')
                                                    <a class="btn btn-warning btn-sm" href="{{ route('comisionado.edit',$comisionado->id) }}"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16">
                                                        <path d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.5.5 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11z"/>
                                                    </svg></a>
                                                @endcan

                                                @can('borrar-comisionado')
                                                    <form action="{{ route('comisionado.destroy', $comisionado->id) }}" method="POST" class="sweetalert" style="margin: 0 3px;">
                                                        @csrf
                                                        {{ method_field('DELETE') }}
        ...
        <script src="{{ asset('vendor/datatables/dataTables.bootstrap4.min.js') }}"></script>
                ...
                <script>
                    ...
                    function updatePdfLink() {
                        ...
                        var pdfUrl = `{{ url('comisionado/pdf') }}?search=${encodeURIComponent(searchTerm)}`;
