            ...
            <div class="col-lg-12">
                <div class="card mb-4">
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                        <h2 class="font-weight-bold text-primary" style="margin-left: 38%;">Gestión de Usuario</h2>
                            @can('crear-usuario')
                        ...
                        <table class="table align-items-center table-flush" id="dataTable">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th class="font-weight-bold text-Secondary">E-mail</th>
                                    <th class="font-weight-bold text-Secondary">Usuario</th>
                                    <th class="font-weight-bold text-Secondary">Rol</th>
                            ...
                            <tbody>
                                @foreach ($usuarios as $usuario)
                                    <tr>
                                        <!-- <td class="font-weight-bold text-Secondary">{{ $usuario->id}}</td> -->
                                        <td class="font-weight-bold text-Secondary">{{ $usuario->name }}</td>
                                        <td class="font-weight-bold text-Secondary">{{ $usuario->email }}</td>
                                        <td class="font-weight-bold text-Secondary">{{ $usuario->username }}</td>
                                        <td>
                                            @if(!empty($usuario->getRoleNames()))
                                            @foreach($usuario->getRoleNames() as $rolNombre)                                       
                                                <span class="font-weight-bold text-Secondary">{{ $rolNombre }}</span>
                                            @endforeach
                                            @endif
                                        </td>
                                        <td>
                                            @can('editar-usuario')
                                                <a class="btn btn-warning btn-sm" style="margin-left: 31%;" href="{{ route('usuarios.edit',$usuario->id) }}"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16">
